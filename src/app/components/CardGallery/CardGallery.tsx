// src/app/components/CardGallery/CardGallery.tsx
'use client'
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-white">🃏 カードギャラリー</h2>
          <button
            onClick={onClose}
        ...
        <div className="bg-white/5 rounded-xl p-4 mb-6">
          <h3 className="text-white font-semibold mb-3">アニメーションテスト</h3>
          
          {/* サイズ選択 */}
          <div className="mb-4">
            <label className="text-white text-sm block mb-2">カードサイズ:</label>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
            ...
            <button
              ...
              className="px-3 py-2 rounded text-sm bg-blue-500 hover:bg-blue-600 text-white transition-all"
            >
              フリップ
            </button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              ...
              className="px-3 py-2 rounded text-sm bg-green-500 hover:bg-green-600 text-white transition-all"
            >
              正解アニメ
            </button>
            ...
            <button
              ...
              className="px-3 py-2 rounded text-sm bg-red-500 hover:bg-red-600 text-white transition-all"
            >
              不正解アニメ
            </button>
            ...
            <button
              ...
              className="px-3 py-2 rounded text-sm bg-gray-500 hover:bg-gray-600 text-white transition-all"
            >
              リセット
            </button>
          </div>
        ...
        {selectedCard && (
          <div className="bg-white/5 rounded-xl p-6 mb-6 text-center">
            <h3 className="text-white font-semibold mb-4">プレビュー</h3>
        ...
        <div className="bg-white/5 rounded-xl p-4 mt-6">
          <h3 className="text-white font-semibold mb-3">📊 カード統計</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-blue-300">
                {CHEMICAL_CARDS.length}
              </div>
              <div className="text-white/70 text-sm">総カード数</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-green-300">
                {CHEMICAL_CARDS.filter(c => c.unit === 'g').length}
              </div>
              <div className="text-white/70 text-sm">分子量カード</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-yellow-300">
                {CHEMICAL_CARDS.filter(c => c.unit === 'mol').length}
              </div>
              <div className="text-white/70 text-sm">mol数カード</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-purple-300">
                {CHEMICAL_CARDS.filter(c => c.unit === 'L').length}
              </div>
              <div className="text-white/70 text-sm">体積カード</div>
            </div>
          </div>
        </div>
        ...
        <div className="text-center mt-6">
          <button
            onClick={onClose}
            className="bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700 text-white font-semibold py-2 px-6 rounded-full transition-all duration-300 hover:scale-105"
          >
            閉じる
          </button>
        </div>
